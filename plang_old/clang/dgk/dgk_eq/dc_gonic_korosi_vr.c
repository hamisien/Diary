...
void gonic_korosi(char* _gid, char* _gonic, int cid, int _vnum)
{
	...
	int i = 0;
	char trigger_p[50] = "CENSORED", trigger_n[50] = "CENSORED", trigger_l[50] = "CENSORED";
...
NO_CID:
		...
		char CENSORED = 0; // 0이면 없음, 1은 사랑해, 2는 싫어
		...
		while(fgets(buff, sizeof(buff), fp) != NULL){
			...
			if (strsearch = strstr(buff, "gallview_contents")) {
				...
				if (CENSORED == 3) { // 만약 어떤 고닉이 "CENSORED" 키워드를 입력했다면 대상 고닉을 우선으로.
