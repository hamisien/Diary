...
get_vnum:
		...
		system("touch buffer"); //curl로 다운로드할 웹페이지의 내용을 저장할 buffer 파일을 만듦.

		do{
			sprintf(cmd_buffer, "curl -s -L m.dcinside.com/board/%s/%ls | grep -i \"derror/deleted\" > buffer", gid, pid);
			system(cmd_buffer);

			memset(buff, 0, sizeof(buff)); //string.h 의 memset 함수로 buff[500]의 이전 대입 값을 0으로 초기화해줌.
			FILE *fp = fopen("buffer", "r");
			fgets(buff, sizeof(buff), fp);

			char* strsearch = strstr(buff, "derror/deleted");
			
			if (strsearch != NULL) {
				system("clear");
				printf("게시글 %ls이(가) 존재하지 않습니다.\n", pid);
				printf("해당 게시글이 작성될 때까지 기다리는 중 ...\a\n\n");
				printf("strsearch값: %s\n", strsearch);
				
				check_vnum_validation = 1;

				system("sleep 0.5");
				//delay(500);
			}
			else check_vnum_validation = 0;
			
			//printf("%s", buff);

			fclose(fp);
		} while(check_vnum_validation == 1);

re:
        for(i=0; i<vnum/2; i++){
                system("clear");
                printf("실행 횟수: %d\n", 2*i+2); //C컴파일러는 기본적으로 우에서 좌로 인식하기에 괄호와 같은 우선연산기호가 없어도 된다.
                sprintf(cmd_buffer, "curl -k -l -o /dev/null -s -L m.dcinside.com/board/%s/%ls & >&-", gid, pid); //-s 옵션은 --silent로, 무엇도 출력하지 않는다. 또한, >&-는 출력값을 &-(?)에 전달하여 출력하라는 것으로, >는 리디렉션 기호이다. &-에 대해선 좀더 알아보자. 첨엔wm3으로하려고했다가무리인거같아서wget으로하려했다가다운이돼서결국curl로하기로함.curl굿굿.
                system(cmd_buffer);
        }
